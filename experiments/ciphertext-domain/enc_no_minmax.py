...
def evaluate_lfw(embeddings: np.ndarray,
    ...
    inputset = generate_inputset(10, (2**bits)-1, pca_dim, 0)
    ...
    for fold in tqdm.tqdm(range(n_folds)):
        ...
        for i in range(n_pairs):
            res = circuit.simulate(quantized1[i], quantized2[i])
            dists.append(res)
...
if __name__ == "__main__":
    ...
    for mm_strategy in mms:
        for q_strategy in qs:
            ...
            df.to_csv(f"./ciphertext-domain/results/fold_eval_no_minmax_{mm_strategy}_{q_strategy}.csv", index=False)
            print(f"\nSaved results to fold_eval_no_minmax_{mm_strategy}_{q_strategy}.csv")
